...
def update_vehicle(vehicle_id: str, payload: VehicleUpdate, db: Session = Depends(get_db)) -> VehicleRead:
...
def update_driver(driver_id: str, payload: DriverUpdate, db: Session = Depends(get_db)) -> DriverRead:
...
def list_allocations(db: Session = Depends(get_db)) -> list[AllocationRead]:
    return FleetAllocationService(db).list_allocations()


@router.post("/orders/{order_id}/allocate", response_model=MessageResponse)
def allocate_order(order_id: str, db: Session = Depends(get_db)) -> MessageResponse:
    try:
